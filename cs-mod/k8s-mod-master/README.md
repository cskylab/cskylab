...
Machine `k8s-mod-master` is deployed from template Ubuntu server 20.04 kubernetes node version 21.12.20.
...
If ssh key has not been injected before, you must provide the password for username `kos@k8s-mod-master` twice:
...
To run scripts and operate from inside the machine, establish an ssh connection with administrator (sudoer) user name `kos@k8s-mod-master`.
