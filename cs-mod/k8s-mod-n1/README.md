# k8s-mod-n1 <!-- omit in toc -->

[comment]: <> (**Machine functional description goes here**)

Machine `k8s-mod-n1` is deployed from template Ubuntu server 20.04 kubernetes node version 21.12.20.
...
If ssh key has not been injected before, you must provide the password for username `kos@k8s-mod-n1` twice:
...
To run scripts and operate from inside the machine, establish an ssh connection with administrator (sudoer) user name `kos@k8s-mod-n1`.
...
| `machine.hostname`           | hostname                                         | `k8s-mod-n1`                                  |
...
| `machine.ipaddress`          | main IP address                                  | `192.168.82.11`                                 |
