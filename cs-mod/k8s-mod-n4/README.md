...
Machine `k8s-mod-n4` is deployed from template Ubuntu server 20.04 kubernetes node version 22-03-23.
...
If ssh key has not been injected before, you must provide the password for username `kos@k8s-mod-n4` twice:
...
To run scripts and operate from inside the machine, establish an ssh connection with administrator (sudoer) user name `kos@k8s-mod-n4`.
...
| `machine.hostname`           | hostname                                         | `k8s-mod-n4`                                  |
...
| `machine.ipaddress`          | main IP address                                  | `192.168.82.14`                                 |
