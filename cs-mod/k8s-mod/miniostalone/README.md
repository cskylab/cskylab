...
## Helm charts: bitnami/minio v12.4.1<!-- omit in toc -->
...
Configuration files are deployed from template MinIO standalone version 23-04-27.
...
- API URL: `miniostalone.mod.cskylab.net`
...
&& echo \
&& ssh kos@k8s-mod-n1.cskylab.net \
  'sudo cs-rsync.sh -q -m rsync-to -d /srv/miniostalone \
...
```bash
################################################################################
# /srv/miniostalone - RSync LVM data services
################################################################################
##
...
&& echo \
&& ssh kos@k8s-mod-n1.cskylab.net \
  'sudo cs-restic.sh -q -m restic-bck -d  /srv/miniostalone -t miniostalone' \
&& echo \
&& echo "******** END of snippet execution ********" \
&& echo
```
...
&& echo \
&& ssh kos@k8s-mod-n1.cskylab.net \
  'sudo cs-restic.sh -q -m restic-list  -t miniostalone' \
&& echo \
&& echo "******** END of snippet execution ********" \
&& echo
```
