...
- bitnami/postgresql 13.2.16 appVersion 15.x (image selected in values-posgresql.yaml).
...
Configuration files are deployed from template Kubernetes Keycloak namespace version 23-11-24.
...
- Published at: `keycloakx.pro.cskylab.net/auth`
- Username: `keycloak`
- Password: `NoFear21`
...
| ---------------------------- | ---------------------------- | ------------------------------- |
| `/srv/keycloakx` | `k8s-pro-n1` | `k8s-pro-n2` |
...
# 0 8-23 * * *     root run-one cs-lvmserv.sh -q -m snap-remove -d /srv/keycloakx >> /var/log/cs-rsync.log 2>&1 ; run-one cs-rsync.sh -q -m rsync-to -d /srv/keycloakx -t k8s-pro-n2.cskylab.net  >> /var/log/cs-rsync.log 2>&1
```
...
```bash
kubectl -n keycloakx exec -i postgresql-0 -- /bin/bash -c "PGPASSWORD='NoFear21' pg_dumpall -U postgres" > postgresql.dump
```
...
```bash
kubectl -n keycloakx exec -i postgresql-0 -- /bin/bash -c "PGPASSWORD='NoFear21' psql -U postgres" < postgresql.dump
```
