...
Machine `kvm-aux` is deployed from template Ubuntu server 20.04 kvm base installation version 21.12.19.
...
If ssh key has not been injected before, you must provide the password for username `kos@kvm-aux` twice:
...
To run scripts and operate from inside the machine, establish an ssh connection with administrator (sudoer) user name `kos@kvm-aux`.
...
| `_tplversion`                | template version                                 | `21.12.19`                                       |
