...
# Charts
helm pull jetstack/cert-manager --version 1.16.2 --untar
...
if [[ "${execution_mode}" == "install" ]]; then
  ...
  echo
  echo "${msg_info} Create namespace ${namespace}"
  echo
...
if [[ "${execution_mode}" == "update" ]] ||
  [[ "${execution_mode}" == "apply-cliss" ]]; then

  # Apply clusterissuers
  echo
  echo "${msg_info} Apply clusterissuers manifests"
  echo

  shopt -s nullglob
  declare -a cliss_files=(./clusterissuer-*.yaml)
  shopt -u nullglob
  count=${#cliss_files[@]}
  if ((count > 0)); then
    for file in "${cliss_files[@]}"; do
      kubectl -n "${namespace}" apply -f "${file}"
    done
  fi

  echo
  echo "${msg_info} Clusterissuers manifests applied"
  echo
...
if [[ "${execution_mode}" == "uninstall" ]]; then
  ...
  # Delete clusterissuers if present
  echo
  echo "${msg_info} Delete clusterissuers manifests"
  echo
  ...
  echo
  echo "${msg_info} Clusterissuers manifests deleted"
  echo
...
if [[ "${execution_mode}" == "list-status" ]] ||
  ...
  # Display Clusterissuers status
  echo
  echo "${msg_info} Clusterissuers status information"
  echo
  kubectl -n "${namespace}" get clusterissuers
