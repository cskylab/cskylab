...
#   csdeploy.sh
#
#       GitLab namespace {{ .namespace.name }}.
#
...
# Charts
helm pull gitlab/gitlab --version 5.4.1 --untar
helm pull bitnami/postgresql --version 10.13.5 --untar

EOF
)"

# Password for secrets
secret_pass="{{ .publishing.password }}"
# Gitlab URL
pub_url="{{ .publishing.url }}"
# Custom CA file.pem
...
if [[ "${execution_mode}" == "install" ]]; then
  ...
  kubectl -n="${namespace}" create secret generic gitlab-initial-root-password \
    --from-literal=password="${secret_pass}"
  # Get root password
  # kubectl -n=${namespace} get secret gitlab-initial-root-password -ojsonpath='{.data.password}' | base64 --decode ; echo

  # Postgresql password
  kubectl -n="${namespace}" create secret generic gitlab-postgresql-password \
    --from-literal=postgresql-password="${secret_pass}" \
    --from-literal=postgresql-postgres-password="${secret_pass}"

  # redis password
  kubectl -n="${namespace}" create secret generic gitlab-redis-password \
    --from-literal=password="${secret_pass}"

  # GitLab CustomCA's secrets
  # kubectl -n=${namespace} create secret generic customca-secret --from-file=${custom_ca_file}
  kubectl -n="${namespace}" create secret generic customca-secret --from-file="${pub_url}.crt"=${custom_ca_file}
  # Letsencrypt staging root
  kubectl -n="${namespace}" create secret generic carootlestaging-secret --from-file="${pub_url}.crt"=fakelerootx1.pem
  # Letsencrypt production root
  # kubectl -n="${namespace}" create secret generic carootleprod-secret --from-file="${pub_url}.crt"=isrgrootx1.pem

  # keycloak-secret
  kubectl -n="${namespace}" create secret generic gitlab-keycloak-secret --from-file=provider=keycloak-provider.yaml

  # Create Certificates form cert-*.yaml manifests
  shopt -s nullglob
  declare -a cert_files=(./cert-*.yaml)
  shopt -u nullglob
  count=${#cert_files[@]}
  if ((count > 0)); then
    echo
    echo "${msg_info} Create certificates"
    echo
    for file in "${cert_files[@]}"; do
      kubectl -n "${namespace}" apply -f "${file}"
    done
    echo
    echo "${msg_info} Certificates created:"
    echo
  fi

  # Create Persistent Volumes from pv-*.yaml manifests
  shopt -s nullglob
  declare -a pv_files=(./pv-*.yaml)
  shopt -u nullglob
  count=${#pv_files[@]}
  if ((count > 0)); then
    echo
    echo "${msg_info} Create PV's"
    echo
    for file in "${pv_files[@]}"; do
      kubectl -n "${namespace}" apply -f "${file}"
    done
    echo
    echo "${msg_info} PV's created"
    echo
  fi
...
if [[ "${execution_mode}" == "remove" ]] ||
  ...
  if ((count > 0)); then
    echo
    echo "${msg_info} Delete PV's"
    echo
    for file in "${pv_files[@]}"; do
      kubectl -n "${namespace}" delete -f "${file}"
    done
    echo
    echo "${msg_info} PV's deleted"
    echo
  fi
fi
...
if [[ "${execution_mode}" == "list-status" ]] ||
  ...
  echo
  echo "${msg_info} Certificate status information"
  echo
  kubectl -n "${namespace}" get certificates.cert-manager.io

  # Display secret status
  echo
  echo "${msg_info} Secret status information"
  echo
  kubectl -n "${namespace}" get secrets

  # Display persistence status
  echo
  echo "${msg_info} Persistent volume claims status information"
  echo
