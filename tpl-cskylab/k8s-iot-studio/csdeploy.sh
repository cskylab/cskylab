...
## Charts
helm pull k8s-at-home/mosquitto --version 4.3.1 --untar
helm pull k8s-at-home/node-red --version 10.1.1 --untar
helm pull influxdata/influxdb2 --version 2.0.12 --untar
helm pull bitnami/grafana --version 7.6.33 --untar
...
################################################################################
# Create namespace, config-maps, secrets and PV's
################################################################################
...
################################################################################
# Deploy app mod's and charts
################################################################################

if [[ "${execution_mode}" == "update" ]] ||
  [[ "${execution_mode}" == "install" ]]; then

  # Check for app module manifests in the form mod-*.yaml
  shopt -s nullglob
  declare -a mod_files=(./mod-*.yaml)
  shopt -u nullglob
  count=${#mod_files[@]}
  if ((count > 0)); then
    echo
    echo "${msg_info} Apply mod's manifests"
    echo
    for file in "${mod_files[@]}"; do
      kubectl -n "${namespace}" apply -f "${file}"
    done
    echo
    echo "${msg_info} Mod's manifests applied"
    echo
  fi
...
################################################################################
# Uninstall charts and app mod's
################################################################################
...
if [[ "${execution_mode}" == "uninstall" ]]; then
  ...
  if ((count > 0)); then
    echo
    echo "${msg_info} Delete mod's manifests"
    echo
    for file in "${mod_files[@]}"; do
      kubectl -n "${namespace}" delete -f "${file}"
    done
    echo
    echo "${msg_info} Mod's manifests deleted"
    echo
  fi
