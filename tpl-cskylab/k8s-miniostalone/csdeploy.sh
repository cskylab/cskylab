...
# Charts
helm pull bitnami/minio --version 12.10.1 --untar

EOF
)"

## MinIO Root Credentials
minio_accesskey="{{ .credentials.minio_accesskey }}"
minio_secretkey="{{ .credentials.minio_secretkey }}"
...
################################################################################
# Create namespace, secrets and PV's
################################################################################
...
if [[ "${execution_mode}" == "install" ]]; then
  ...
  kubectl -n="${namespace}" create secret generic minio-creds-secret \
    --from-literal=root-user="${minio_accesskey}" \
    --from-literal=root-password="${minio_secretkey}"
